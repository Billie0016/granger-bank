 ...
 *
 * This file intentionally has no "server-only" import, unlike rbac.ts which
 * re-exports it. Next app code should keep importing from "./rbac" (or
 * "@/server/auth/rbac") to keep that tripwire against accidental
 * client-bundle inclusion. This module exists so prisma/seed.ts — a
 * standalone script run via tsx, entirely outside Next's runtime, where the
 * "server-only" package's real implementation throws unconditionally
 * regardless of client/server intent — can still call grantScopes without
 * duplicating its transaction logic.
 */
