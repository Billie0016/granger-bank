 ...
 *
 * This file intentionally has no "server-only" import, unlike env.ts which
 * re-exports it — see rbacCore.ts for why that split exists. Next app code
 * should keep importing from "./env" (or "@/server/env") to keep the real
 * enforcement (not just convention) behind "no secret ever reaches the
 * browser" — see docs/production/06-banking-provider-integration.md §5.
 * This module exists only so prisma/seed.ts (a standalone tsx script,
 * outside Next's runtime, where "server-only" throws unconditionally) can
 * reach isProduction() transitively via dbCore.ts.
 */
